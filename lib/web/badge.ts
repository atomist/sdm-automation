...
function badgeRequestHandler(config: Configuration) {
    return async (req, res, next) => {
        ...
        if (badge && badge.SdmGoalSetBadge && badge.SdmGoalSetBadge.length > 0) {
            const bRepo = badge.SdmGoalSetBadge[0].repo;

            if (bRepo.name !== repo || bRepo.owner !== owner) {
                res.sendStatus(404);
                return next();
            }
            ...
            if (goalSet && goalSet.SdmGoal && goalSet.SdmGoal.length > 0) {
                ...
                logger.info(`Sent badge for '${owner}/${repo}/${branch}' with token '${token}'`);
                return next();
            }
            res.sendStatus(404);
